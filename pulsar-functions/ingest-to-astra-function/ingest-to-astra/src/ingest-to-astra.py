...
class IngestToAstraFunction(Function):
    ...
    def process(self, input, context):
        ...
        try:
            collection.find_one_and_replace({"_id": data["_id"]}, data, upsert=True,)
            context.get_logger().info(
                f"Data with message id {context.get_message_id()} successfully inserted into Astra DB collection {collection.name}.")
        except Exception as e:
